...
if (isset($_GET['id'])) {
    ...
    if ($categoria->setId($_GET['id'])) {
        ...
        if ($rowCategoria = $categoria->readOne() ) {
            ...
            $pdf->startReport('Productos de la categoría '.$rowCategoria['nombres_cliente']);
            ...
            if ($dataProductos = $categoria->readOrdenes()) {
                ...
                $pdf->Cell(94, 10, utf8_decode('Nombre producto'), 1, 0, 'C', 1);
                ...
                foreach ($dataProductos as $rowCategoriass) {
                    ...
                    $pdf->Cell(94, 10, utf8_decode($rowCategoriass['nombre_producto']), 1, 0, 'C', 1);
                    $pdf->Cell(46, 10, utf8_decode($rowCategoriass['precio_producto']), 1, 0, 'C', 1);
                    $pdf->Cell(46, 10, utf8_decode($rowCategoriass['cantidad_producto']), 1, 1, 'C', 1);
                    
                    
                }
                if ($rowCategorias = $categoria->total()) {
                    $pdf->SetFont('Arial', 'B', 15);
                    $pdf->Cell(280, 30, utf8_decode('Total de compra (+IVA): $'.$rowCategorias['total']), 0, 1, 'C');
                }
            } 
